...
export default function ReportsPage() {
  ...
  const loadReports = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Something went sideways — let’s try again", error instanceof Error ? error.message : "Couldn’t load reports.");
    } finally {
      setLoading(false);
    }
  }, [toast, user?.clientId]);
    ...
    <section key={pathname} className={`page-section overview-grid ${pageClassName}`}>
      {loading ? (
        ...
        <Card className="report-hero-card">
          <div className="section-heading">
            <div>
              <p style={{ margin: 0, color: "var(--amber)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>Latest report</p>
              <h2 style={{ fontSize: "2rem" }}>{currentMonthTitle}</h2>
        ...
        <Card className="section-card">
          <EmptyState illustration="reports" title="Your first report is on its way" description="PulseOS generates a polished PDF report as your conversations start flowing in." />
        </Card>
      )}
      ...
      <Card className="section-card">
        ...
        {loading ? (
          ...
          <EmptyState illustration="reports" title="No monthly history yet" description="As soon as customer conversations start flowing in, your monthly timeline will appear here." />
        )}
      </Card>
