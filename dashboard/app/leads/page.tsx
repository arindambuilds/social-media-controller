...
export default function LeadsPage() {
  ...
  return (
    <div key={pathname} className={`page-shell ${pageClassName}`}>
      ...
      {error ? <ErrorState message="Couldn’t load leads" detail={error} onRetry={() => window.location.reload()} /> : null}
      ...
      <section className="gradient-border mt-6 p-4 md:p-6">
        {leads.length === 0 && !error ? (
          <EmptyState
            ...
            subline="When comments or DMs look like real buying intent, they will show up here so you can follow up fast."
          />
        ) : leads.length > 0 ? (
            ...
            {leads.map((lead) => {
              ...
              return (
                <StaggerItem key={lead.id}>
                  <div className={`lead-row-card flex flex-col gap-4 px-4 py-4 ${rowGrid}`}>
