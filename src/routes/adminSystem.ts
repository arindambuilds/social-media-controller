...
adminSystemRouter.get("/system", async (_req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : "Failed to load system snapshot.";
    res.status(500).json({ success: false, error: { code: "SYSTEM_DASHBOARD_ERROR", message } });
  }
});
