...
async function closeRedisGracefully(): Promise<void> {
...
function registerRuntimeShutdownHooks(): void {
  const standardWorkers = [
    workerBriefing,
    workerPdf,
    workerMaintenance,
    workerWhatsAppBriefing,
    workerInstagramSync
  ].filter((worker): worker is Worker => worker != null);

  if (standardWorkers.length > 0) {
    registerShutdownHook(async () => {
      await Promise.all(standardWorkers.map((worker) => worker.close()));
    });
  }
  ...
  registerShutdownHook(() => closeRedisGracefully());
}
